...
.DEFAULT_GOAL := help
.PHONY: greet blog calculator computer run-api help
project := greet calculator blog computer
...
computer: $@ ## Generate obs and build for computer
#run-api: $@ ## Generate obs and build for all
...
clean: clean_greet clean_calculator clean_blog clean_computer ## Clean generated files
	${RM_F_CMD} ssl/*.crt
	${RM_F_CMD} ssl/*.csr
	${RM_F_CMD} ssl/*.key
	${RM_F_CMD} ssl/*.pem
...
clean_computer: ## Clean generated files for computer
	${RM_F_CMD} computer/${PROTO_DIR}/*.pb.go
